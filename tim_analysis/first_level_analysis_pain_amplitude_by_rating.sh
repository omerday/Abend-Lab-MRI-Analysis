...
task() {
    ...
    time=$(date +"%H").$(date +%M)
    old_results_path=${1}.$session_prefix.pain.rating.results
    ...
    if [ -d ~/Dropbox/${1}.pain.rating.old ]; then
        rm -r ~/Dropbox/${1}.pain.rating.old
    fi

    echo "Backing up previous results on Dropbox"
    if [ -d ~/Dropbox/${1}.pain.rating ]; then
        mv ~/Dropbox/${1}.pain.rating ~/Dropbox/${1}.pain.rating.old
    fi

    mkdir ~/Dropbox/${1}.pain.rating
    ...
    afni_proc.py \
        ...
        -regress_stim_times \
            ${input_folder}/${1}/${session_prefix}/func/timings/pain_rating_amp.1D \
        -regress_stim_labels PAIN \
        ...
        -write_3dD_prefix stats.${1}.nii \
        -execute

    echo "Backing up QC to Dropbox"
    mkdir ~/Dropbox/${1}.pain.rating
    cp -R ${1}.results/QC_${1} ~/Dropbox/${1}.pain.rating/QC
    ...
    cp -R ${1}.results/chauffeur ~/Dropbox/${1}.pain.rating/chauffeur

    echo "Moving results to sub-folder by session"
    mv ${1}.results ${output_folder}/${1}.${session_prefix}.pain.rating.results
    echo "Done"
}
