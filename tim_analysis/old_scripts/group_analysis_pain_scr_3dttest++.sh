...
for subject in ${subject_ids[@]}; do
    mask_epi_anat_files="${mask_epi_anat_files} ${input_folder}/${subject}.ses-1.pain.scr.results/mask_epi_anat.*+tlrc.HEAD"
    dsets="${dsets} ${subject} ${input_folder}/${subject}.ses-1.pain.scr.results/stats.${subject}+tlrc[SCR#1_Coef]"
done
...
3dttest++ -prefix 3dttest_TIM_fMRI_PAIN_SCR \
    -setA SCR#1 ${dsets}

3dcalc  -a 3dttest_TIM_fMRI_PAIN_SCR+tlrc.HEAD \
        -b group_mask_olap.4+tlrc.HEAD \
        -expr 'a*b' \
        -prefix 3dttest_TIM_fMRI_PAIN_SCR_masked
...
@chauffeur_afni                                                         \
    ...
    -ulay_range         0% 130%                                         \
    -olay               ./3dttest_TIM_fMRI_PAIN_SCR_masked+tlrc.HEAD    \
