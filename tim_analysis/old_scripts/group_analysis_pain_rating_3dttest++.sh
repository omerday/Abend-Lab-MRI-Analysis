...
for subject in ${subject_ids[@]}; do
    mask_epi_anat_files="${mask_epi_anat_files} ${input_folder}/${subject}.ses-1.pain.rating.results/mask_epi_anat.*+tlrc.HEAD"
    dsets="${dsets} ${subject} ${input_folder}/${subject}.ses-1.pain.rating.results/stats.${subject}+tlrc[PAIN#1_Coef]"
done
...
3dttest++ -prefix 3dttest_TIM_fMRI_PAIN_RATING \
    -setA PAIN#1 ${dsets}

3dcalc  -a 3dttest_TIM_fMRI_PAIN_RATING+tlrc.HEAD \
        -b group_mask_olap.4+tlrc.HEAD \
        -expr 'a*b' \
        -prefix 3dttest_TIM_fMRI_PAIN_RATING_masked
...
@chauffeur_afni                                                         \
    ...
    -ulay_range         0% 130%                                         \
    -olay               ./3dttest_TIM_fMRI_PAIN_RATING_masked+tlrc.HEAD    \
    ...
    -olay_alpha         Yes                                             \
    -olay_boxed         Yes                                             \
    -set_subbricks      -1 "PAIN#1_Tstat" "PAIN#1_Tstat"              \
    -opacity            5                                               \
    -zerocolor          white                                           \
    -prefix             chauffeur/PAIN/RATING                  \
