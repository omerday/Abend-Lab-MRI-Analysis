...
task() {
    ...
    time=$(date +"%H").$(date +%M)
    old_results_path=${1}.$session_prefix.antic.1234.results
    ...
    if [ -d ~/Dropbox/${1}.antic.1234.old ]; then
        rm -r ~/Dropbox/${1}.antic.1234.old
    fi

    echo "Backing up previous results on Dropbox"
    if [ -d ~/Dropbox/${1}.antic.1234 ]; then
        mv ~/Dropbox/${1}.antic.1234 ~/Dropbox/${1}.1234.old
    fi

    mkdir ~/Dropbox/${1}.antic.1234
    ...
    afni_proc.py \
        ...
        -regress_stim_times \
            ${input_folder}/${1}/${session_prefix}/func/timings/anticipation_1234_amp.1D \
    ...
    mkdir ~/Dropbox/${1}.antic.1234
    cp -R ${1}.results/QC_${1} ~/Dropbox/${1}.antic.1234/QC
    ...
    cp -R ${1}.results/chauffeur ~/Dropbox/${1}.antic.1234/chauffeur

    echo "Moving results to sub-folder by session"
    mv ${1}.results ${output_folder}/${1}.${session_prefix}.antic.1234.results
    echo "Done"
}
